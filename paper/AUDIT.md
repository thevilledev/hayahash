...
| Rust, Go, Zig, Java, C#, WebAssembly, and pure JavaScript implementations pass their checked-in conformance vectors at this snapshot. | reproduced, archived | `paper/results/conformance.txt`; current tests in each port | A single generated machine-readable vector file is still needed. The present suites share copied tables. |
