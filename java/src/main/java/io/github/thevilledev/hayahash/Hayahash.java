 ...
 * and seed on every platform, and is independent of host endianness.
 *
 * <p>The reference's portability rules (no SIMD, no 128-bit multiply, no hardware-specific
 * instructions) map directly onto cheap JVM primitives: Java {@code long} arithmetic wraps modulo
 * 2<sup>64</sup> exactly like C {@code uint64_t}, {@link Long#rotateLeft} and {@code >>>} cover the
...
public final class Hayahash {
   ...
   *
   * <p>Produces exactly the same value as the C reference {@code hayahash64()} for all inputs,
   * seeds, and host endiannesses.
   *
  ...
  /**
   * Hashes the {@code length} bytes of {@code key} starting at {@code offset} with {@code seed},
   * returning a 64-bit digest.
   *
   * <p>Equivalent to hashing a copy of that range, without the copy; useful when keys live inside
   * larger buffers.
   *
   ...
   * @param seed the seed
   * @return the 64-bit digest
   * @throws IndexOutOfBoundsException if {@code [offset, offset + length)} is not a valid range of
   *     {@code key}
   */
